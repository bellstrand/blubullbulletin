...
class CMUser extends CObject implements IHasSQL, ArrayAccess, IModule{
   ...
   */
		...
		switch($action){
			case 'install':
				try{
					...
					return array('success', 'Successfully created the database tables and created a default admin user as root:root and an ordinary user ad doe:doe.');
				}
				catch(Exception$e){
	...
	*
	* @param string $acronymOrEmail the email-address or user acronym.
	* @param string $password the password that should match the acronym or email-address.
